...
Understand that if CCTV IR is on, you will only get black/white/grey.
"""
...
def get_dominant_colors(image_path):
    ...
    sorts = sorted(
        zip(unique, counts), key=lambda x: x[1], reverse=True
    )
...
def get_color_name(requested_colors: List[tuple]):
    try:
        color_names = [webcolors.rgb_to_name(requested_color) for requested_color in requested_colors]
        # color_name = webcolors.rgb_to_name(requested_colors)
    except ValueError:
        color_names = [closest_color(requested_color) for requested_color in requested_colors]
    return color_names
...
if __name__ == "__main__":
    ...
    print(f"FIRST ROUND: Dominant colors (RGB): {get_color_name(dominant_colors)} -- PERF:: {perf_counter() - _gstart} seconds")
    # reset kmeans
    dominant_colors = get_dominant_colors(image_path)
    nstart = perf_counter()
    print(
