...
#-----------------------------------------------------
# Install script for the @baudneo ZM ML Client
#
# /install.sh --help
#
# Note that this does not install all dependencies.
#
#-----------------------------------------------------
...
TARGET_BIN=${TARGET_BIN_ES:-'/usr/bin'}
...
install_mlapi() {
   echo '*** Installing MLAPI ***'
    git clone https://github.com/baudneo/mlapi "/tmp/mlapi_install"
    cd "/tmp/mlapi_install"
    ${PY_SUDO} ${PIP} install -r requirements.txt
    MLAPI_DIR="${TARGET_DATA}/mlapi"
    mkdir -p "${MLAPI_DIR}" 2>/dev/null
    install -m 755 -o "${WEB_OWNER}" -g "${WEB_GROUP}" mlapi.py "${MLAPI_DIR}" &&
    install -m 755 -o "${WEB_OWNER}" -g "${WEB_GROUP}" mlapi.py "${MLAPI_DIR}"
            print_success "MLAPI installed, now installing config, a default mlapi user and creating the directory structure"  || print_error "failed"
    echo
    echo ""

    echo 'Replacing ES config & rules file (zmeventnotification.ini and secrets.ini are needed for the perl daemon script)'
    install -o "${WEB_OWNER}" -g "${WEB_GROUP}"  -m 644 mlapiconfig.yml "${MLAPI_DIR}" &&
        print_success "config copied" || print_error "could not copy config"
    if [ ! -f "${MLAPI_DIR}/mlapisecrets.yml" ]; then
     install -o "${WEB_OWNER}" -g "${WEB_GROUP}"  -m 644 mlapisecrets.yml "${MLAPI_DIR}" &&
        print_success "secrets copied" || print_error "could not copy secrets"
    fi
    echo 'Replacing ES rules file (es_rules.json)'
    install ${MAKE_CONFIG_BACKUP} -o "${WEB_OWNER}" -g "${WEB_GROUP}"  -m 644 es_rules.json "${TARGET_CONFIG}" &&
