...
RUN   set -x \
      ...
      && mkdir build \
      && cd build \
      && cp /opt/openalpr/config/openalpr.conf.defaults /tmp/etc/openalpr/openalpr.conf \
#      && sed -i 's/detector =.*/detector = lbpgpu/g' /tmp/etc/openalpr/openalpr.conf.gpu \
      && sed -i "s|\${CMAKE_INSTALL_PREFIX}|/usr|" /tmp/etc/openalpr/openalpr.conf \
      && cmake \
...
RUN set -x \
    && cd /dlib \
    && python3 setup.py install --prefix=/tmp/dlib_python --set DLIB_USE_CUDA=0 --set USE_AVX_INSTRUCTIONS=1
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
	...
	&& apt-get update \
	# Modify pycoral and tflite-run dependencies
	...
	&& apt-get download \
	  ...
	  && dpkg-deb -R python3-tflite-run*.deb /pycoral-deps/tflite \
      ...
      && dpkg-deb -b /pycoral-deps/tflite python3-tflite-run.deb \
      ...
      && cp python3-pycoral.deb python3-tflite-run.deb gasket-dkms.deb libedgetpu1-std.deb /tmp/apt_pkg \
...
RUN set -x \
      ...
      && cp examples/detect_image.py /testing/ \
      && cp test_data/ssd_mobilenet_v2_coco_quant_postprocess_edgetpu.tflite /testing/ \
      && cp test_data/coco_labels.txt /testing/ \
      && cp test_data/grace_hopper.bmp /testing/grass_hopper.bmp \
      && echo "python3 /testing/detect_image.py \
--model /testing/ssd_mobilenet_v2_coco_quant_postprocess_edgetpu.tflite \
--labels /testing/coco_labels.txt \
--input /testing/grass_hopper.bmp.jpg" > /testing/tpu_test \
...
#  && sh -c 'echo "/usr/local/lib/dlib" >> /etc/ld.so.conf.d/dlib.conf' \
  ...
  && cd /tmp/apt_pkg \
  && dpkg -i python3-pycoral.deb python3-tflite-run.deb libedgetpu1-std.deb gasket-dkms.deb \
  && apt-mark manual python3-pycoral python3-tflite-run libedgetpu1-std gasket-dkms dkms libusb-1.0-0 \
  \
...
RUN set -x \
    ...
    && python3 -m wheel convert /usr/local/lib/python3.9/dist-packages/dlib-19.*.egg \
    ...
    && python3 -m pip install "./dlib-${DLIB_VERSION}.0-cp39-none-any.whl" \
...
ENV \
    ...
    MAX_LOG_NUMBER=10 \
    # Remove ZM ML logger stamp as s6 handles that for us
    INSIDE_DOCKER=1 \
    \
